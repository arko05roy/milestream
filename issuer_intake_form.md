# HACD Incubator Cohort 2 — Issuer Intake Form

## 1. Project basics

- Project name: MileStream
- Ticker / asset symbol: MSTR
- Category: utility
- One-sentence description: MileStream is a contractor-credential Stack Asset that pairs PoW-backed identity with GrowStreams milestone payments so freelancers get paid for small gigs.
- Founder / team: Arko Roy (solo founder)
- Website: https://github.com/arko05roy
- X / community links: @notarkoroy
- Contact: itsarko619@gmail.com

## 2. Why HACD

- Why should this asset be issued on HACD instead of a normal token contract? Freelancer credentials need anti-spam economics and portable on-chain proof. A plain token mint has zero formation cost and no unique identity per holder.
- What does PoW-backed formation add to this project? Formation cost filters fake profiles. PoW-mined HACD containers make credentials costly to replicate at scale.
- Why does the asset need HACD containers? Each contractor credential is tied to one HACD lot and one unique Contractor ID. Supply is hard-capped at 200 founding contractors.
- What should users remember after seeing this launch? MileStream credentials are formed, not deployed. Your Contractor ID is your HACD name, permanently on-chain.

## 3. Asset design

- Asset type: HYBRID (FT + NFT)
- Total supply: 1000000
- Number of HACD lots: 200
- Units per HACD lot: 5000
- Are all lots equal? Yes, within each phase. 20 pilot lots + 180 public lots.
- Does removing the stack burn / disable / unlock anything? Removing the stack burns 5,000 MSTR per lot and retires the Contractor ID.

## 4. Stack cost

- Proposed stack cost per HACD in HAC: 50 HAC
- Who receives or controls the stack cost? Stack cost goes to the Hacash network protocol per Stack mechanism rules.
- Is any HAC burned, locked, paid, or reserved? HAC is paid as formation cost per protocol. First phase reserves 20 lots for pilot contractors only.
- Estimated network fee: standard Hacash network transaction fee per lot

## 5. Launch rules

- Launch date target: 2026-07-01
- Public sale / allowlist / designated address / phased launch: designated_first (20 pilot, 180 public)
- Minimum HACD per participant: 1
- Maximum HACD per participant: 5
- Is there a first phase reserved for issuer or designated address? Yes — 20 lots for sprint pilot contractors
- When does public participation begin? After pilot phase validation, within the same launch window ending 2026-07-01

## 6. Utility

- What can holders do at launch? Verify MSTR balance and Contractor ID on the Hacash explorer and HACD Launchpad.
- What may holders do during the sprint? Contractor directory listing and GrowStreams milestone templates (in development).
- What is already built? Complete 8-document issuance package; validated launch_spec.json.
- What depends on future development? Contractor directory web app, GrowStreams pilot gigs, client review system.
- What should not be promised publicly? Investment returns, token price appreciation, guaranteed client volume, licensed escrow status.

## 7. Community and communication

- Target audience: Freelancers doing $50–$500 gigs; clients hiring on Telegram/Discord; remote workers
- Main narrative: Get paid. Prove you're real. Form your credential on HACD, stream payments on GrowStreams.
- Tone: practical / problem-solving
- Key announcement message: MileStream launches contractor credentials on HACD with milestone payments on GrowStreams.
- Three things you want users to understand:
  1. MSTR is a credential, not an investment.
  2. Payments flow through GrowStreams, not the token contract.
  3. Formation cost makes fake profiles expensive.

## 8. Risk and compliance

- Legal review required: Yes (payment-adjacent product)
- Risk disclosures prepared: Yes
- Promises to avoid: yield, guaranteed income, price floor, licensed escrow claims

## 9. Campaign links

- GrowStreams quest: https://www.growstreams.xyz/app/projects/HACD
- HACD incubator application: https://hacd.it/incubator
- HACD Launchpad: https://hacd.it/launchpad

---

*Completed intake for Arko Roy. Issuer confirmation pending final parameter sign-off.*
